...
import { TransientConsumerError } from '../../messaging/consumer/consumer.errors';

@Injectable()
export class EmailProcessedMessageRepository {
  private readonly consumerName = 'banking-backend-email';
  ...
  async markCompleted(messageId: string): Promise<void> {
    ...
    if (Number(updated) !== 1) {
      throw new TransientConsumerError(
        `failed to finalize email processed message as COMPLETED messageId=${messageId}`,
      );
    }
  }
  ...
  async markFailed(messageId: string, errorMessage: string): Promise<void> {
    try {
      ...
      this.structuredLogger.error(
        EmailProcessedMessageRepository.name,
        'Failed to mark email processed message as FAILED',
        {
          details: {
            eventType: 'EMAIL',
            action: 'MARK_FAILED',
